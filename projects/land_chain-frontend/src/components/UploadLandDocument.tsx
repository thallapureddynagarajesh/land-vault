    ...
    <div className="space-y-5">
      ...
      <div className="glass-card p-4 sm:p-5 rounded-2xl border border-stone-200/90 relative overflow-hidden shadow-xs">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-stone-900">Upload & Register Land Document</h2>
              <p className="text-xs text-stone-500 mt-0.5">
                Upload physical deed PDFs to <strong>IPFS</strong> & seal immutable CIDs + SHA-256 metadata on <strong>Algorand</strong>.
              </p>
            </div>
          </div>
          ...
          <div className="bg-stone-50 px-3 py-1.5 rounded-lg border border-stone-200 text-[11px] font-mono text-stone-600">
            ...
            <span className="text-earth-700 font-semibold">IPFS Docs • Algorand AVM</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="glass-card p-4 sm:p-5 rounded-2xl border border-stone-200 space-y-4">
            <div className="flex items-center justify-between border-b border-stone-200 pb-3">
              <h3 className="text-lg font-bold text-stone-800 flex items-center gap-2">
                <FileCode2 className="w-5 h-5 text-earth-600" /> Land Record Metadata & Document Upload
              </h3>
              <span className="text-xs font-mono text-earth-700 bg-earth-50 px-2.5 py-1 rounded-lg border border-earth-200">
                Pinata IPFS Enabled
              </span>
            </div>

            {errorMessage && (
              <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-600 text-xs space-y-2">
            ...
            <form onSubmit={handleUploadAndRegister} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="text-xs font-semibold text-stone-600 block mb-1 flex items-center gap-1.5">
                    <Hash className="w-3.5 h-3.5 text-earth-600" /> Property ID (Parcel PIN)
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl glass-input text-xs text-stone-700 placeholder-stone-400 font-mono"
                  />
                </div>

                <div>
                  <label className="text-xs font-semibold text-stone-600 block mb-1 flex items-center gap-1.5">
                    <FileCheck className="w-3.5 h-3.5 text-amber-600" /> Survey Number
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-xs font-semibold text-stone-600 block mb-1 flex items-center gap-1.5">
                    <Layers className="w-3.5 h-3.5 text-earth-600" /> Document Type
                  </label>
                  <select
                    value={documentType}
                    onChange={(e) => setDocumentType(e.target.value)}
                    className="w-full px-3.5 py-2.5 rounded-xl glass-input text-xs text-stone-700 focus:outline-none bg-white cursor-pointer"
                  >
              ...
              <div>
                <label className="text-xs font-semibold text-stone-600 block mb-1 flex items-center gap-1.5">
                  <MapPin className="w-3.5 h-3.5 text-earth-500" /> Location / City Address
                </label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl glass-input text-xs text-stone-700 placeholder-stone-400"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-stone-600 block mb-1 flex items-center gap-1.5">
                    <FileCheck className="w-3.5 h-3.5 text-stone-500" /> Plot Area (Square Feet)
                  </label>
                  <input
                    type="number"
                    required
                ...
                <div>
                  <label className="text-xs font-semibold text-stone-600 block mb-1 flex items-center gap-1.5">
                    <User className="w-3.5 h-3.5 text-earth-600" /> Owner Algorand Wallet Address
                  </label>
                  <input
                    type="text"
                    required
              ...
              <div className="space-y-1.5 pt-2">
                <label className="text-xs font-semibold text-stone-600 block mb-1 flex items-center gap-1.5">
                  <UploadCloud className="w-3.5 h-3.5 text-amber-600" /> Select Land Document (PDF / Images / Docs)
                </label>
                <div className="p-4 rounded-2xl border-2 border-dashed border-stone-300 bg-stone-50 hover:bg-white transition-all text-center space-y-2 relative">
                  <input
                    type="file"
                  ...
                  />
                  <UploadCloud className="w-8 h-8 text-earth-500 mx-auto" />
                  {selectedFile ? (
                    <div>
                      <span className="text-xs font-bold text-earth-700 block">{selectedFile.name}</span>
                    ...
                    <div>
                      <span className="text-xs font-semibold text-stone-700 block">Click or Drop PDF Land Deed File Here</span>
                      <span className="text-[11px] text-stone-400">PDF, PNG, JPG, TXT, DOC (Max 10MB limit)</span>
                    </div>
                  )}
                </div>
                {fileError && <span className="text-xs text-rose-500 font-semibold block">{fileError}</span>}
              </div>
              ...
              {step === 'BLOCKCHAIN_FAILED' && stagedCid ? (
                <button
                  ...
                  className="w-full py-3.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs shadow-lg shadow-amber-500/15 flex items-center justify-center gap-2 cursor-pointer transition-all"
                >
                ...
                <button
                  type="submit"
                  disabled={step !== 'IDLE'}
                  className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-earth-700 via-earth-600 to-earth-500 hover:from-earth-600 hover:to-earth-400 text-white font-extrabold text-xs shadow-lg shadow-earth-600/15 flex items-center justify-center gap-2 cursor-pointer transition-all disabled:opacity-50"
                >
        ...
        <div className="lg:col-span-5 space-y-6">
          {step === 'SUCCESS' && stagedCid ? (
            <div className="glass-card p-6 rounded-3xl border border-green-200 space-y-5 bg-white shadow-lg">
              <div className="flex items-center gap-3 border-b border-stone-200 pb-3">
                <CheckCircle2 className="w-8 h-8 text-green-600 shrink-0" />
                <div>
                  <h3 className="text-base font-bold text-stone-800">Registration Successful!</h3>
                  <p className="text-xs text-green-700 font-mono">Confirmed on Algorand Box Storage</p>
                </div>
              </div>

              <div className="space-y-3 text-xs font-mono">
                <div>
                  <span className="text-stone-400 block text-[11px]">Property ID</span>
                  <span className="font-bold text-stone-800 text-sm">{propertyId}</span>
                </div>

                {x402ProofTx && (
                  <div>
                    <span className="text-stone-400 block text-[11px]">x402 HTTP 402 Payment Receipt (0.005 ALGO)</span>
                    <div className="p-2.5 rounded-xl bg-stone-50 border border-green-200 text-green-700 break-all text-[11px] flex items-center justify-between">
                      <span>{x402ProofTx}</span>
                      <span className="font-bold text-[10px] bg-green-100 px-2 py-0.5 rounded text-green-800">0.005 ALGO Paid</span>
                    </div>
                  </div>
                )}

                <div>
                  <span className="text-stone-400 block text-[11px]">IPFS Content Identifier (CID)</span>
                  <div className="p-2.5 rounded-xl bg-stone-50 border border-stone-200 text-earth-700 break-all text-[11px]">
                    {stagedCid}
                  </div>
                </div>

                <div>
                  <span className="text-stone-400 block text-[11px]">Cryptographic SHA-256 Document Hash</span>
                  <div className="p-2.5 rounded-xl bg-stone-50 border border-stone-200 text-green-700 break-all text-[11px]">
                    {stagedHash}
                  </div>
                </div>

                <div>
                  <span className="text-stone-400 block text-[11px]">Algorand Transaction ID</span>
                  <div className="p-2.5 rounded-xl bg-stone-50 border border-stone-200 text-amber-700 break-all text-[11px]">
                    {resultTxId}
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-stone-200 space-y-2">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full py-2.5 rounded-xl bg-earth-600 hover:bg-earth-500 text-white font-semibold text-xs flex items-center justify-center gap-2 transition-colors"
                >
                ...
                {onSuccessNavigate && (
                  <button
                    ...
                    className="w-full py-2.5 rounded-xl bg-stone-100 hover:bg-stone-200 text-stone-700 font-semibold text-xs flex items-center justify-center gap-2 cursor-pointer transition-colors"
                  >
            ...
            <div className="glass-card p-6 rounded-3xl border border-stone-200 space-y-4">
              <h3 className="text-base font-bold text-stone-800 flex items-center gap-2 border-b border-stone-200 pb-3">
                <ShieldCheck className="w-5 h-5 text-earth-600" /> Decentralized Architecture Principles
              </h3>

              <div className="space-y-4 text-xs text-stone-600">
                <div className="p-4 rounded-2xl bg-stone-50 border border-stone-200 space-y-1">
                  <span className="font-bold text-earth-700 block flex items-center gap-1">
                ...
                <div className="p-4 rounded-2xl bg-stone-50 border border-stone-200 space-y-1">
                  <span className="font-bold text-earth-600 block flex items-center gap-1">
