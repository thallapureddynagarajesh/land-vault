        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Manage your certified real-estate holdings, track verification status, issue sale listings, or transfer ownership on Algorand.</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900/90 px-4 py-2 rounded-xl border border-slate-800 text-xs font-mono text-slate-300">
            ...
            <span className="text-emerald-400 font-semibold">{connectedAddress ? `${connectedAddress.slice(0, 6)}...${connectedAddress.slice(-4)}` : 'Demo Landowner'}</span>
          </div>
        </div>
      </div>
            ...
            <div key={parcel.parcelId} className="glass-card p-6 rounded-3xl border border-slate-800 space-y-5 flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs bg-slate-950/70 p-3.5 rounded-2xl border border-slate-800/80">
                  <div>
                    ...
                    <span className="font-mono font-bold text-amber-300">{parcel.surveyNumber || `SURVEY-${parcel.parcelId}`}</span>
                  </div>
                  <div>
              ...
              <div className="space-y-2 pt-2 border-t border-slate-800">
                {parcel.status === 'VERIFIED' ? (
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-[11px] text-slate-400 text-center font-semibold">
              ...
              {listingParcelId === parcel.parcelId && (
                <form onSubmit={handleListSubmit} className="p-4 bg-slate-950 rounded-2xl border border-amber-500/30 space-y-3">
                  <span className="text-xs font-bold text-amber-300 block">List Land Parcel on Marketplace</span>
                  <input
                    type="number"
                    required
                    step="0.001"
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setListingParcelId(null)} className="flex-1 py-1.5 rounded-lg bg-slate-800 text-slate-400 text-xs">
                      Cancel
                    </button>
              ...
              {transferParcelId === parcel.parcelId && (
                <form onSubmit={handleTransferSubmit} className="p-4 bg-slate-950 rounded-2xl border border-teal-500/30 space-y-3">
                  <span className="text-xs font-bold text-teal-300 block">Transfer Title Ownership</span>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-3 py-2 rounded-xl glass-input text-xs text-white font-mono text-[11px] placeholder-slate-500"
                  />
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setTransferParcelId(null)} className="flex-1 py-1.5 rounded-lg bg-slate-800 text-slate-400 text-xs">
                      Cancel
                    </button>
                    <button type="submit" className="flex-1 py-1.5 rounded-lg bg-teal-600 hover:bg-teal-500 text-white font-bold text-xs">
                      Execute Transfer
                    </button>
                  </div>
                </form>
